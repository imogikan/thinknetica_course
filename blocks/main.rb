...
class Trip
  ...
  def emulate_program
    ...
    set_route_to_train
    train = trains.first
    carriages.each do |carr|
      train.take_carriage(carr)
    end

    train.all_carriages { |carriage| p carriage }

    stations.each do |station|
      station.all_trains_on_the_station(:pass) { |st| p st }
    end

    carriages.first.take_capacity
  end
  ...
  def create_train
    number = get_user_data(type: :name) { "Insert number of train:" }
  ...
  def create_carriage
    ...
    case type
    when 1
      sitting_places = get_user_data { "Insert count of sitting places:" }
      carriage = CarriageWrapper.create_carriage(:pass, capacity, sitting_places)
    when 2
      volume = get_user_data { "Insert volume of carriage:" }
      carriage = CarriageWrapper.create_carriage(:cargo, volume)
    end
